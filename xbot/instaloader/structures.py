...
class Hashtag:
    ...
    @classmethod
    def from_name_v2(cls, context: InstaloaderContext, name: str):
        """
        Create a Hashtag instance from a given hashtag name, without preceeding '#'. Raises an Exception if there is no
        hashtag with the given name.
    ...
    def _query_v2(self, params):
        result=self._context.get_json("explore/tags/{0}/".format(self.name),params)
        #result1=result["graphql"]
        result1=result["data"]
        return result1                           
    def _obtain_metadata(self):
        if not self._has_full_metadata:
    ...
    def _obtain_metadata_v2(self):
        if not self._has_full_metadata:
            self._node = self._query_v2({"__a": 1})
            self._has_full_metadata = True
    def _asdict(self):
        json_node = self._node.copy()
        # remove posts
    ...
    #ahmetcan
    def get_media(self) -> Iterator[Post]:
        """Yields the top posts of the hashtag."""
        yield from (Edge(edge["node"])
                    for edge in self._metadata("edge_hashtag_to_top_posts", "edges"))
        yield from (Edge(edge["node"])
                    for edge in self._metadata("edge_hashtag_to_media", "edges"))

    
    def get_media_v2(self) -> Iterator[Post]:
        """Yields the top posts of the hashtag."""
        for section in self._metadata("top","sections"):
            medias=section["layout_content"]["medias"]
            for media in medias:
                yield Edge(media["media"])
...
class Edge:
    ...
    @property
    def id(self) -> str:
        return self._node['id'] if 'id' in self._node else ""
    
    @property
    def username(self) -> str:
        return self._node['username'] if 'username' in self._node else ""
    @property
    def comments(self) -> int:
        return self._node['edge_media_to_comment']["count"] if 'edge_media_to_comment' in self._node else self._node.get('comment_count',0)

    @property
    def likes(self) -> int:
        total= self._node['edge_liked_by']["count"] if 'edge_liked_by' in self._node else  self._node.get('like_count',0)
        total+= self._node['edge_media_preview_like']["count"] if 'edge_media_preview_like' in self._node else 0
        return total
    @property
    def owner_id(self) -> int:
        return self._node['owner']["id"] if 'owner' in self._node else  self._node["user"]["pk"]
      
    @property
    def product_type(self) -> str:
        return self._node['product_type'] if 'product_type' in self._node else ""
    
    @property
    def is_video(self) -> bool:
        return self._node['is_video'] if 'is_video' in self._node else False
        
    @property
    def date_utc(self) -> datetime:
        return datetime.utcfromtimestamp(self._node["date"]
                                         if "date" in self._node
                                         else self._node["taken_at_timestamp"]) if "taken_at_timestamp" in self._node else (datetime.utcnow()-timedelta(days=1)) #self._node["device_timestamp"]
